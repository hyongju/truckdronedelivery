...
% this script varies the relative speed of drones/trucks
addpath('../lib')
...
maxRad = 0.4/annArborSize;
relSpeed = [0.5 1 1.5 2 3];     % SPEED OF DRONES / SPEED OF TRUCK default: 1
% relEff = 5:5:20;    % RELATIVE EFFICIENCY OF DRONES OVER TRUCK
relEff = 20;
ptr = 0;
for u1 = 1:length(relSpeed)
    u1
    for u2 = 1:length(relEff)
        %u2
        [timeCon{u1,u2},feulCon{u1,u2},nDronesCap1{u1,u2},nDronesCap2{u1,u2}] = pathGenAnnArbor(nCustomers,nTrucks,maxRad,relEff(u2),relSpeed(u1),relSpeed(u1));
        % mTSP (randomized)
        % mTSP (k-mean)
...
cset2 = relSpeed;
str = 'speed ratio (UAVs to UGVs) = \{';
for i = 1:length(cset2)
    if i ==length(cset2)
        str = strcat(str,sprintf('%.1f',cset2(i)),'\}');
    else
        str = strcat(str,sprintf('%.1f',cset2(i)),',');
    end
end
mxfC = min(max(max(fC)),50);
for j=1:size(tC,2)
